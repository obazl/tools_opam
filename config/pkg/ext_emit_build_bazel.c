                               ...
                               obzl_meta_entries *_entries)
{
    TRACE_ENTRY;
    (void)level;
    LOG_TRACE(0, "_pkg_name: %s", _pkg_name);
                            ...
                            /* char *_subpkg_dir) */
{
    ...
    for (int i = 0; i < obzl_meta_entries_count(entries); i++) {
        ...
        if (entry->type == OMP_PACKAGE) {
            ...
            /* emit buildfiles for @opam.foo module */
...
EXPORT void ext_emit_build_bazel(// char *ws_name,
                             UT_string *opam_switch_lib,
                             char *coswitch_lib,
                             int level,
                             char *_pkg_root,
                             ...
                             /* char *_subpkg_dir) */
{
    ...
    log_debug("xmkdir_r: %s", utstring_body(build_bazel_file));
                              ...
                              /* char *runfiles) */
{
    ...
    emit_ocaml_bigarray_pkg(NULL, switch_lib, coswitch_lib);
