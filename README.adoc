...
Currently the only tool is the opam module extension.
...
=== Global
You can also use the current global switch, even if you have a local
switch, by editing `MODULE.bazel` and changing `toolchain="local"`
to `toolchain="global"`.

=== XDG

Finally, you can tell the extension to create the entire toolchain
...
opam configuration. They will be shared across OBazl projects using
`toolchain = "xdg".
