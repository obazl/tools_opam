...
EXPORT void opam_xdg_refresh(char *_opam_switch_name)
{
    ...
    if (debug) {
        log_info("opening coswitch bootstrapper: %s",
                  utstring_body(bootstrap_filename));
        printf("opening coswitch bootstrapper: %s\n",
               utstring_body(bootstrap_filename));
    }
    ...
    opam_coswitch_set(utstring_body(opam_switch_name));
}
